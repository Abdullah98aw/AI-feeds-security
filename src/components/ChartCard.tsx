export function BarChartCard({ title, data }: { title: string; data: Array<{ label: string; value: number }> }) {
  ...
  return (
    <section className="min-w-0 rounded-lg border border-line bg-panel p-5">
      <h2 className="text-base font-semibold leading-6 text-slate-100">{title}</h2>
      <div className="mt-5 space-y-4">
        {data.map((item) => (
          <div key={item.label} className="grid grid-cols-[minmax(0,1fr)_2.5rem] items-center gap-3 text-sm sm:grid-cols-[minmax(8rem,11rem)_minmax(5rem,1fr)_2.5rem]">
            <span className="min-w-0 leading-5 text-slate-400">{item.label}</span>
  ...
  return (
    <section className="min-w-0 rounded-lg border border-line bg-panel p-5">
      ...
      <svg viewBox="0 0 300 140" className="mt-4 min-h-44 w-full overflow-visible">
        <polyline fill="none" stroke="#50e3c2" strokeWidth="4" strokeLinejoin="round" strokeLinecap="round" points={points.join(' ')} />
        {data.map((item, index) => {
          const x = (index / Math.max(data.length - 1, 1)) * 300;
          const y = 110 - (item.value / max) * 92;
          return (
            <g key={item.label}>
              <circle cx={x} cy={y} r="4" fill="#50e3c2" />
              <text x={x} y="134" textAnchor="middle" className="fill-slate-500 text-[11px]">
                {item.label}
              </text>
            </g>
          );
