import { Bell, Languages, Moon, Search, ShieldCheck, Sun } from 'lucide-react';
...
export function TopBar() {
  ...
  return (
    <header className="sticky top-0 z-20 border-b border-line bg-graphite backdrop-blur">
      <div className="mx-auto flex max-w-7xl flex-wrap items-center gap-2 px-4 py-3 pl-16 sm:flex-nowrap sm:gap-3 sm:px-6 lg:px-8">
        <div className="group order-3 relative min-w-0 flex-none basis-[calc(100vw-5rem)] sm:order-none sm:flex-1 sm:basis-auto">
          <label className="relative block">
            ...
            <input
              className="min-h-11 w-full rounded-xl border border-line bg-[var(--color-input-background)] py-2 pl-10 pr-4 text-sm leading-5 text-[var(--color-input-text)] outline-none transition placeholder:text-[var(--color-input-placeholder)] focus:border-[var(--color-focus-ring)]"
        ...
        <button onClick={() => safeNavigate('/notifications', navigate)} className="relative grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-line bg-panel text-signal hover:border-signal" title={t.notifications}>
          <Bell size={18} />
        ...
        <button onClick={toggleTheme} className="inline-flex min-h-11 w-11 shrink-0 items-center justify-center gap-2 rounded-xl border border-line bg-panel px-0 py-2 text-sm text-signal hover:border-signal sm:w-auto sm:px-3" title={theme === 'light' ? 'Dark Mode' : 'Light Mode'} aria-label={language === 'ar' ? (theme === 'light' ? 'الوضع الداكن' : 'الوضع الفاتح') : (theme === 'light' ? 'Dark Mode' : 'Light Mode')}>
          {theme === 'light' ? <Moon size={18} /> : <Sun size={18} />}
          <span className="hidden sm:inline">{theme === 'light' ? 'Dark' : 'Light'}</span>
        </button>
        <button onClick={() => setLanguage(language === 'en' ? 'ar' : 'en')} className="inline-flex min-h-11 w-11 shrink-0 items-center justify-center gap-2 rounded-xl border border-line bg-panel px-0 py-2 text-sm text-signal hover:border-signal sm:w-auto sm:px-3" title="Language">
          <Languages size={18} />
          <span className="hidden sm:inline">{language === 'en' ? 'AR' : 'EN'}</span>
        </button>
        <div className="hidden min-h-11 shrink-0 items-center gap-2 rounded-xl border border-line bg-panel px-3 py-2 text-sm text-slate-300 sm:flex">
